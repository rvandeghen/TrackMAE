...
from engine_for_pretraining import train_one_epoch, visualize_reconstruction
...
def get_args():
    ...
    parser.add_argument('--mask_ratio_decoder', default=0.0, type=float,
                        help='ratio of the encoded visual tokens/patches need be masked in decoder')
    ...
    parser.add_argument('--wandb_entity', type=str, default="vlmae")
    parser.add_argument('--wandb_key', type=str, default=None)
    parser.add_argument('--wandb_name', type=str, default=None)

    parser.add_argument('--visualize_freq', default=1, type=int)
...
def get_model(args):
    ...
    if args.use_cls_token:
        model_kwargs["use_cls_token"] = True
...
def main(args, wandb_run=None):
    ...
    if global_rank == 0 and args.log_dir is not None:
        ...
        log_writer = None #utils.TensorboardLogger(log_dir=args.log_dir)
    else:
        log_writer = None
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # Visualization step
        if (not args.skip_visualization and (epoch + 1) % args.visualize_freq == 0 and utils.is_main_process()):
            # Use the same prefetcher for visualization, reset to ensure fresh batch
            prefetcher.reset()
            save_path = os.path.join(args.output_dir, f"visualizations/epoch_{epoch+1}") if args.output_dir else ""
